...
export const SUBSIDIES_BATCH_8: Subsidy[] = Array.from({ length: 100 }, (_, i) => {
  const index = i + 701;
  const id = `SUB-${String(index).padStart(3, '0')}`;
  const sector = 'Renewable Energy & Solar';
